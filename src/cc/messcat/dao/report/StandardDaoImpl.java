...
public class StandardDaoImpl extends BaseDaoImpl implements StandardDao {
	...
	public Standard get(Long id) {
		return (Standard) getHibernateTemplate().get(Standard.class, id);
	}

	public List findAll() {
		return getHibernateTemplate().find("from Standard where status = 1 order by editTime desc");
	}
	 ...
	 * 根据条件查询
	 */
	public List<Standard> findByhql(String string) {
