...
public class AuthenticationDaoImpl extends BaseDaoImpl implements AuthenticationDao {
	...
	public Authentication get(Long id) {
		return (Authentication) getHibernateTemplate().get(Authentication.class, id);
	}

	public List findAll() {
		return getHibernateTemplate().find("from Authentication where status = 1 order by editTime desc");
	}
